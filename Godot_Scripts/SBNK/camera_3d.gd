...
var radius := 35  # Radius of the circle
var center := Vector3(0, 10, 0)  # The point around which the camera rotates
var step := Vector3(0, 0.2, 0) # Steps for camera position updates
...
var rotation_speed_degrees := 6  # Rotation speed in degrees per second
...
var initial_configs = [
	{ "position": Vector3(0.8, -0.8, 35), "rotation": Vector3(-20, 0, 0) } #y = -0.8 to 0.8
]
